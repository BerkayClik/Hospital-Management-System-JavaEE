...
public class MakeAppServlet2 extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(date.length() != 21 || doctor.length() == 0 || times.length()<14){
...
//        String doctor = "Dogukan Duduoglu";
            ...
            for(int k = 0;k<=howManyDays;k++){
                ...
                for (int j = 0;j<howManyHours;j++){
                    ...
                    try {
                        ...
                        while(appCheck.next()){
                            unavailableTimes.add(appCheck.getString(1));
                        }
                ...
                if(c1.get(c1.DAY_OF_WEEK)!=1) {
                    ...
                    for (int j = 0; j < howManyHours; j++) {
                        ...
                        if (unavailableTimes.size() != 0) {
                            if ((date + " " + times).equals(unavailableTimes.get(n).substring(0, 16))) {
                                html += "<div class=\"res\">\n" +
            ...
            html += "</div>" +
                    "<input type=\"text\" name=\"selectedTime\" value=\"\" id=\"resTime\" style=\"display: none\">\n" +
                    "                       <br>\n" +
            ...
            request.setAttribute("startTime", "");
            request.setAttribute("endTime", "");
            System.out.println("line214: " + startDate + "/" + endDate);
            System.out.println("line215: " + startTime + " " + endTime);
            request.setAttribute("doctorName", doctorEncodedName);
            request.getRequestDispatcher("makeAppointment2.jsp").forward(request,response);
        }
    }
